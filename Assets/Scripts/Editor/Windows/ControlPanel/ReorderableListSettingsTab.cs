	...
	public static class ReorderableListSettingsTab
	{
		...
		public static void DrawGUI(FoCsControlPanel owner)
		{
			using(FoCsEditor.Disposables.VerticalScope(FoCsGUI.Styles.Unity.Box, GUILayout.ExpandHeight(true), GUILayout.ExpandWidth(true)))
			{
				...
				FoCsEditor.UnityReorderableListProperty.LimitingEnabled = FoCsGUI.Layout.Toggle(FoCsEditor.UnityReorderableListProperty.LimitingEnabled? Enabled : Disabled, FoCsEditor.UnityReorderableListProperty.LimitingEnabled);
				...
				using(var cc = FoCsEditor.Disposables.ChangeCheck())
				{
					...
					var num = FoCsGUI.Layout.IntField("Total Display Count", FoCsEditor.UnityReorderableListProperty.ListLimiter.TOTAL_VISIBLE_COUNT);

					if(cc.changed)
						FoCsEditor.UnityReorderableListProperty.ListLimiter.TOTAL_VISIBLE_COUNT = num;
				}
