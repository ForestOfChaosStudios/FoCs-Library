...
namespace UnityEditor.PostProcessing
{
	...
	{
		private static readonly string[]           s_MethodNames = {"Fast Approximate Anti-aliasing", "Temporal Anti-aliasing"};
		...
		{
			m_Method                = FindSetting((AntialiasingModel.Settings x) => x.method);
			m_FxaaPreset            = FindSetting((AntialiasingModel.Settings x) => x.fxaaSettings.preset);
			m_TaaJitterSpread       = FindSetting((AntialiasingModel.Settings x) => x.taaSettings.jitterSpread);
			m_TaaSharpen            = FindSetting((AntialiasingModel.Settings x) => x.taaSettings.sharpen);
			m_TaaStationaryBlending = FindSetting((AntialiasingModel.Settings x) => x.taaSettings.stationaryBlending);
			m_TaaMotionBlending     = FindSetting((AntialiasingModel.Settings x) => x.taaSettings.motionBlending);
		}
		...
		public override void OnInspectorGUI()
		{
			...
			if(m_Method.intValue == (int)AntialiasingModel.Method.Fxaa)
				EditorGUILayout.PropertyField(m_FxaaPreset);
			else if(m_Method.intValue == (int)AntialiasingModel.Method.Taa)
			{
