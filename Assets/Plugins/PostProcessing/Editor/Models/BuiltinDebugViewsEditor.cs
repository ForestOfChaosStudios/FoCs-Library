...
namespace UnityEditor.PostProcessing
{
	...
	public class BuiltinDebugViewsEditor: PostProcessingModelEditor
	{
		...
		public override void OnEnable()
		{
			...
			m_Depth = new DepthSettings {scale = FindSetting((BuiltinDebugViewsModel.Settings x) => x.depth.scale)};
			...
			m_MotionVectors = new MotionVectorsSettings
			{
					...
					motionImageOpacity      = FindSetting((BuiltinDebugViewsModel.Settings x) => x.motionVectors.motionImageOpacity),
					motionImageAmplitude    = FindSetting((BuiltinDebugViewsModel.Settings x) => x.motionVectors.motionImageAmplitude),
					motionVectorsOpacity    = FindSetting((BuiltinDebugViewsModel.Settings x) => x.motionVectors.motionVectorsOpacity),
					motionVectorsResolution = FindSetting((BuiltinDebugViewsModel.Settings x) => x.motionVectors.motionVectorsResolution),
					motionVectorsAmplitude  = FindSetting((BuiltinDebugViewsModel.Settings x) => x.motionVectors.motionVectorsAmplitude)
			};
		}
		...
		public override void OnInspectorGUI()
		{
			...
			{
				CheckActiveEffect((mode == (int)BuiltinDebugViewsModel.Mode.AmbientOcclusion)                                                     && !profile.ambientOcclusion.enabled, "Ambient Occlusion");
				CheckActiveEffect((mode == (int)BuiltinDebugViewsModel.Mode.FocusPlane)                                                           && !profile.depthOfField.enabled,     "Depth Of Field");
				CheckActiveEffect((mode == (int)BuiltinDebugViewsModel.Mode.EyeAdaptation)                                                        && !profile.eyeAdaptation.enabled,    "Eye Adaptation");
				CheckActiveEffect(((mode == (int)BuiltinDebugViewsModel.Mode.LogLut) || (mode == (int)BuiltinDebugViewsModel.Mode.PreGradingLog)) && !profile.colorGrading.enabled,     "Color Grading");
				CheckActiveEffect((mode == (int)BuiltinDebugViewsModel.Mode.UserLut)                                                              && !profile.userLut.enabled,          "User Lut");
			}
		}
