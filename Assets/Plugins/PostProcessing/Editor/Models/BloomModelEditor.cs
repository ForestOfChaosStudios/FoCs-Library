...
namespace UnityEditor.PostProcessing
{
	...
	public class BloomModelEditor: PostProcessingModelEditor
	{
		...
		public override void OnEnable()
		{
			...
			{
					intensity   = FindSetting((BloomModel.Settings x) => x.bloom.intensity),
					threshold   = FindSetting((BloomModel.Settings x) => x.bloom.threshold),
					softKnee    = FindSetting((BloomModel.Settings x) => x.bloom.softKnee),
					radius      = FindSetting((BloomModel.Settings x) => x.bloom.radius),
					antiFlicker = FindSetting((BloomModel.Settings x) => x.bloom.antiFlicker)
			};

			m_LensDirt = new LensDirtSettings {texture = FindSetting((BloomModel.Settings x) => x.lensDirt.texture), intensity = FindSetting((BloomModel.Settings x) => x.lensDirt.intensity)};
		}
...
#region Graph
		...
		private void DrawRect(float x1, float y1, float x2, float y2, float fill, float line)
		{
			...
			Handles.DrawSolidRectangleWithOutline(m_RectVertices, fill < 0? Color.clear : Color.white * fill, line < 0? Color.clear : Color.white * line);
		}
		...
		public void DrawGraph()
		{
			...
			// Label
			Handles.Label(PointInRect(0, m_RangeY) + Vector3.right, "Brightness Response (linear)", EditorStyles.miniLabel);
