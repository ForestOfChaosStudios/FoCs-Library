	...
	public class ColorGradingModelEditor: PostProcessingModelEditor
	{
		...
		public override void OnEnable()
		{
			...
			m_ChannelMixer = new ChannelMixerSettings
			{
					...
					currentEditingChannel = FindSetting((ColorGradingModel.Settings x) => x.channelMixer.currentEditingChannel)
			};
		...
		{
			var tid = EditorGUILayout.Popup(EditorGUIHelper.GetContent("Tonemapper"), m_Tonemapping.tonemapper.intValue, s_Tonemappers);
		...
		private void DrawNeutralTonemappingCurve()
		{
			...
			// Label
			Handles.Label(PointInRect(0, k_NeutralRangeY) + Vector3.right, "Neutral Tonemapper", EditorStyles.miniLabel);
		...
		private float NeutralCurve(float x, float a, float b, float c, float d, float e, float f) => (((x * ((a * x) + (c * b))) + (d * e)) / ((x * ((a * x) + b)) + (d * f))) - (e / f);
		...
		private void DoChannelMixerGUI()
		{
...

			{
				using(new EditorGUILayout.HorizontalScope())
				{
					...
					if(GUILayout.Toggle(currentChannel == 0, EditorGUIHelper.GetContent("Red|Red output channel."), EditorStyles.miniButtonLeft))
						currentChannel = 0;

					if(GUILayout.Toggle(currentChannel == 1, EditorGUIHelper.GetContent("Green|Green output channel."), EditorStyles.miniButtonMid))
						currentChannel = 1;

					if(GUILayout.Toggle(currentChannel == 2, EditorGUIHelper.GetContent("Blue|Blue output channel."), EditorStyles.miniButtonRight))
						currentChannel = 2;
				}
			}
		...
		private void DoColorWheelsGUI()
		{
			...
			using(new EditorGUILayout.HorizontalScope())
			{
				...
				if(GUILayout.Toggle(wheelMode == (int)ColorGradingModel.ColorWheelMode.Linear, "Linear", EditorStyles.miniButtonLeft))
					wheelMode = (int)ColorGradingModel.ColorWheelMode.Linear;

				if(GUILayout.Toggle(wheelMode == (int)ColorGradingModel.ColorWheelMode.Log, "Log", EditorStyles.miniButtonRight))
					wheelMode = (int)ColorGradingModel.ColorWheelMode.Log;
			}
					...
					if(GUILayout.Button("Reset", EditorStyles.toolbarButton))
					{
						switch(curveEditingId)
						{
							case 0:
								...
								if(y)
									m_Curves.master.animationCurveValue = AnimationCurve.Linear(0f, 0f, 1f, 1f);

								if(r)
									m_Curves.red.animationCurveValue = AnimationCurve.Linear(0f, 0f, 1f, 1f);

								if(g)
									m_Curves.green.animationCurveValue = AnimationCurve.Linear(0f, 0f, 1f, 1f);

								if(b)
									m_Curves.blue.animationCurveValue = AnimationCurve.Linear(0f, 0f, 1f, 1f);
