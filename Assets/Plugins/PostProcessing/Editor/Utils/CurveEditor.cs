...
#region Structs
		public struct Settings
		{
			...
			public static Settings   defaultSettings => new Settings {bounds = new Rect(0f, 0f, 1f, 1f), padding = new RectOffset(10, 10, 10, 10), selectionColor = Color.yellow, curvePickingDistance = 6f, keyTimeClampingDistance = 1e-4f};
		}
		...
		private void OnCurveGUI(Rect rect, SerializedProperty curve, CurveState state)
		{
			...
			for(var k = 0; k < length; k++)
			{
				...
				if(state.editable)
				{
					...
					{
						var alreadyBroken = !(Mathf.Approximately(keys[k].inTangent, keys[k].outTangent) || (float.IsInfinity(keys[k].inTangent) && float.IsInfinity(keys[k].outTangent)));
					...
					if((e.type == EventType.mouseDown) && rect.Contains(e.mousePosition))
					{
						if(hitRect.Contains(e.mousePosition))
						{
							...
							else if(e.button == 1)
							{
								...
								menu.AddItem(new GUIContent("Delete Key"),
								             false,
								             x =>
								             {
		...
		private void OnGeneralUI(Rect rect)
		{
			...
			if(e.type == EventType.mouseDown)
			{
				...
				foreach(var curve in m_Curves)
				{
					...
					var hitY      = animCurve.length == 0? state.zeroKeyConstantValue : animCurve.Evaluate(hit.x);
					...
					if(Mathf.Abs(curvePos.y - curvePickValue) < settings.curvePickingDistance)
					{
						...
						else if(e.button == 1)
						{
							...
							menu.AddItem(new GUIContent("Add Key"),
							             false,
							             x =>
							             {
								             var action     = (MenuAction)x;
...
#region Maths utilities
		private Vector3 CurveToCanvas(Keyframe keyframe) => CurveToCanvas(new Vector3(keyframe.time, keyframe.value));
		...
		private Vector3 CurveTangentToCanvas(float tangent)
		{
			...
			{
				var bounds = settings.bounds;
				var ratio  = m_CurveArea.width / m_CurveArea.height / ((bounds.xMax - bounds.x) / (bounds.yMax - bounds.y));
		...
		private float ProjectTangent(float inPosition, float inValue, float inTangent, float projPosition) => inValue + ((projPosition - inPosition) * inTangent);
