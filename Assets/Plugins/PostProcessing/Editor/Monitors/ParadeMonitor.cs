...
namespace UnityEditor.PostProcessing
{
	public class ParadeMonitor: PostProcessingMonitor
	{
		...
		public override bool IsSupported() => (m_ComputeShader != null) && GraphicsUtils.supportsDX11;
		public override GUIContent GetMonitorTitle() => s_MonitorTitle;
		...
		public override void OnMonitorGUI(Rect r)
		{
			if(Event.current.type == EventType.Repaint)
			{
				...
				// Sizing
				var width  = m_WaveformTexture != null? Mathf.Min(m_WaveformTexture.width,  r.width  - 65f) : r.width;
				var height = m_WaveformTexture != null? Mathf.Min(m_WaveformTexture.height, r.height - 45f) : r.height;
				m_MonitorAreaRect = new Rect(Mathf.Floor((r.x + (r.width / 2f))                      - (width / 2f)), Mathf.Floor((r.y + (r.height / 2f)) - (height / 2f) - 5f), width, height);
		...
		private void ComputeWaveform(RenderTexture source)
		{
			...
			var channels = m_MonitorSettings.waveformY? new Vector4(0f, 0f, 0f, 1f) : new Vector4(m_MonitorSettings.waveformR? 1f : 0f, m_MonitorSettings.waveformG? 1f : 0f, m_MonitorSettings.waveformB? 1f : 0f, 0f);
