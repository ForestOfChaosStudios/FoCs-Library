namespace UnityEngine.PostProcessing
{
	...
	public sealed class ColorGradingComponent: PostProcessingComponentRenderTexture<ColorGradingModel>
	{
		...
		private float StandardIlluminantY(float x) => (2.87f * x) - (3f * x * x) - 0.27509507f;
		...
		private static Color NormalizeColor(Color c)
		{
			...
			return new Color {r = c.r / sum, g = c.g / sum, b = c.b / sum, a = c.a};
		}
		...
		private bool IsLogLutValid(RenderTexture lut) => (lut != null) && lut.IsCreated() && (lut.height == k_InternalLogLutSize);
		...
		private void GenerateLut()
		{
			...
			lutMaterial.SetVector(Uniforms._LutParams, new Vector4(k_InternalLogLutSize, 0.5f / (k_InternalLogLutSize * k_InternalLogLutSize), 0.5f / k_InternalLogLutSize, k_InternalLogLutSize / (k_InternalLogLutSize - 1f)));
			...
			CalculateLiftGammaGain(settings.colorWheels.linear.lift, settings.colorWheels.linear.gamma, settings.colorWheels.linear.gain, out lift, out gamma, out gain);
			...
			CalculateSlopePowerOffset(settings.colorWheels.log.slope, settings.colorWheels.log.power, settings.colorWheels.log.offset, out slope, out power, out offset);
		...
		public override void Prepare(Material uberMaterial)
		{
			...
			uberMaterial.EnableKeyword(context.profile.debugViews.IsModeActive(DebugMode.PreGradingLog)? "COLOR_GRADING_LOG_VIEW" : "COLOR_GRADING");
