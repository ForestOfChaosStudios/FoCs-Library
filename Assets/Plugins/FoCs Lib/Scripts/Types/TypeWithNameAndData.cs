...
namespace ForestOfChaosLib.Types
{
	...
	public static class TypeWithNameAndData
	{
		...
		public class GenericType<T>: BaseGenericType
		{
			...
			public static implicit operator T(GenericType<T> input)
			{
				return input.Data;
			}

			public virtual T GetTypeFromString(string data)
			{
				return default(T);
			}
		...
		public class StringType: GenericType<string>
		{
			...
			public override string GetTypeFromString(string data)
			{
				return data;
			}
